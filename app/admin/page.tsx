    ...
    <section className={`min-w-0 flex h-full min-h-[21rem] flex-col rounded-[1.5rem] p-5 min-[1440px]:col-span-4 ${dashboardTheme.surface}`}>
      ...
      <div className="mt-5 grid grid-cols-2 gap-2.5 sm:grid-cols-4">
        {[
          ...
          ["Validation", data.openAiAnalytics.averageValidationScore ? `${data.openAiAnalytics.averageValidationScore}/100` : "NO DATA"],
          ...
          ["Forecast today", data.openAiAnalytics.predictiveCost.forecastToday ? formatCurrency(data.openAiAnalytics.predictiveCost.forecastToday) : "NO DATA"],
          ["Forecast month", data.openAiAnalytics.predictiveCost.forecastMonth ? formatCurrency(data.openAiAnalytics.predictiveCost.forecastMonth) : "NO DATA"],
          ["Monthly spend", data.openAiAnalytics.predictiveCost.projectedMonthlySpend ? formatCurrency(data.openAiAnalytics.predictiveCost.projectedMonthlySpend) : "NO DATA"],
          ["Burn rate", data.openAiAnalytics.predictiveCost.projectedBurnRate ? `${formatCurrency(data.openAiAnalytics.predictiveCost.projectedBurnRate)}/h` : "NO DATA"],
          ["Cost risk", data.openAiAnalytics.predictiveCost.costRisk.toUpperCase()],
      ...
      {data.openAiAnalytics.adoptionDistribution.length || data.openAiAnalytics.executionRiskDistribution.length ? (
        <div className="mt-3 flex flex-wrap gap-2">
          {data.openAiAnalytics.adoptionDistribution.map((item) => (
            <span key={item.range} className="rounded-full border border-emerald-300/15 bg-emerald-300/5 px-2.5 py-1 text-[11px] text-emerald-100/80">
              {item.range}: {formatCompactNumber(item.count)}
            </span>
          ))}
          {data.openAiAnalytics.executionRiskDistribution.map((item) => (
            <span key={item.risk} className="rounded-full border border-amber-300/15 bg-amber-300/5 px-2.5 py-1 text-[11px] text-amber-100/80">
              {item.risk}: {formatCompactNumber(item.count)}
            </span>
          ))}
        </div>
      ) : null}

      {data.openAiAnalytics.highestPredictedOutcomeReports.length ? (
        <div className="mt-3 flex flex-wrap gap-2">
