...
export function AdminExports({
  ...
  if (variant === "button") {
    return (
      <details className="group relative">
        <summary className="inline-flex h-9 cursor-pointer list-none items-center gap-2 rounded-[0.85rem] border border-teal-300/20 bg-teal-300/10 px-3 text-[11px] font-semibold text-teal-100 transition duration-300 hover:-translate-y-0.5 hover:bg-teal-300/15">
          <Download className="h-3.5 w-3.5" />
          Export Data
        </summary>
        <div className="absolute right-0 top-11 z-40 w-72 rounded-[1.2rem] border border-white/10 bg-black/95 p-3 shadow-2xl shadow-black/40 backdrop-blur-2xl">
          <p className="px-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-500">
            Analytics tables
          </p>
          <div className="mt-2 space-y-1.5">
            {tables.map((table) => (
              <div key={table.id} className="rounded-[0.95rem] border border-white/10 bg-white/[0.045] p-2.5">
  ...
  return (
    <section className="mt-6 rounded-[1.55rem] border border-white/10 bg-white/[0.045] p-5 shadow-2xl shadow-black/25 backdrop-blur-xl">
      ...
      <div className="mt-5 grid gap-3.5 md:grid-cols-2 xl:grid-cols-3">
        {tables.map((table) => (
          <div key={table.id} className="rounded-[1.2rem] border border-white/10 bg-black/25 p-4 transition duration-300 hover:-translate-y-0.5 hover:border-teal-300/25 hover:bg-white/[0.055]">
